...
void app() {
  ...
  while (1) {
    // send_frame(&test,sizeof(uint8_t));
    HAL_Delay(500);
    // printf("%d",test1);
    ...
    /*
            for(int i =0;i < 5 ;i++)
            {
                ...
                {}
                RingBuffer_put(&usart_rx,USART1->DR);
            }
            printf("string is %s",test);

    // HAL_UART_Transmit_DMA(&huart1, uart, 4 );
    ...
    // __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, 256);//red*/
  }
}
